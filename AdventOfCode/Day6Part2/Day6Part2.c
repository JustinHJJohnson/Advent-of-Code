...
int main(int argc, char* argv[])
{
    ...
    int numQuestionsAnswered = 0;							//Number of questions that everyone in a group answered
    //int questionIndex = 0;

    //loop over the input array
	while (inputIndex < inputLength)
	{
		int timesQuestionAnswered[26] = {0};
		int numInGroup = 0;

        //Keep moving through the array till an empty line is found, signalling the end of the current group
		while (inputIndex < inputLength && input[inputIndex][0] != NULL)
		{
			numInGroup++;

            //Loop through the questions each person answered and increased the number of times the question was answered
			for (int i = 0; i < strlen(input[inputIndex]); i++) timesQuestionAnswered[input[inputIndex][i] - 'a']++;
			
			inputIndex++;
		}

		//Loop through the questions and check how many the whole group answered
		for (int i = 0; i < 26; i++)
		{
			if (timesQuestionAnswered[i] == numInGroup) numQuestionsAnswered++;
		}

        //questionIndex++;
        //printf("group %d answered %d questions\n", questionIndex, numQuestionsAnswered);
		inputIndex++;
	}

	printf("There was a total of %d questions answered", numQuestionsAnswered);
};