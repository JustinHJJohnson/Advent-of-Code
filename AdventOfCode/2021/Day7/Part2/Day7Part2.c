...
// Add all the numbers before this number to this number
inline static long long int summation(long long int num)
{
	return (num * (num + 1)) / 2;
}
...
int main(int argc, char* argv[])
{
	...
	long long int leastFuel = 10000000000;								// The amount of fuel used in the most efficent solution
	
	for (int i = 0; i < inputLength; i++) if (intInput[i] > maxValue) maxValue = intInput[i];	// Get the max value

	for (int i = 0; i <= maxValue; i++)
	{
		long long int fuelUsed = 0;

		for (int j = 0; j < inputLength; j++) fuelUsed += summation(abs(intInput[j] - i));

		if (fuelUsed < leastFuel) leastFuel = fuelUsed;
	}

	printf("Least amount of fuel used to align the crabby boys is %lld\n", leastFuel);
