...
int main(int argc, char* argv[])
{
	...
	int numRows = sizeof(input) / sizeof(char*);	// The length of the input array
	int numColumns = strlen(input[0]);
	bool change = true;
	bool foundOccupied = false;
	bool numOccupied = 0;
	int iterations = 0;
	char** tempInput = (char**)malloc(sizeof(input));

	for (int i = 0; i < numRows; i++) tempInput[i] = (char*)malloc(strlen(input[i]) * sizeof(char));
	...
	while (change)
	{
		...
		for (int i = 0; i < numRows; i++) 
		{
			tempInput[i] = (char*)malloc(strlen(input[i]) * sizeof(char));
			strcpy(tempInput[i], input[i]);
			// printf("%s\n", input[i]);
		}
			...
			for (int j = 0; j < numColumns; j++)
			{
				...
				if (tempInput[i][j] == 'L')
				{
					for (int k = i - 1; k <= i + 1; k++)
					{
						...
						{
							if (k >= 0 && k < numRows && l >= 0 && l < numColumns && tempInput[k][l] == '#') foundOccupied = true;
						}
					}

					if (!foundOccupied) input[i][j] = '#';
				}
				else if (tempInput[i][j] == '#')
				{
					// printf("Occupied found\n");
					...
					for (int k = i - 1; k <= i + 1; k++)
					{
						...
						{
							// if (k == j && l == i);
							// else if (k >= 0 && k < numRows && l >= 0 && l < numColumns && tempInput[k][l] == '#') numOccupied++;
							if (k >= 0 && k < numRows && l >= 0 && l < numColumns && tempInput[k][l] == '#') numOccupied++;
						}
					}

					printf("%d %d num changed %d\n", i, j, numOccupied);

					if (numOccupied >= 4) 
					{
						input[i][j] = 'L';
						printf("changed %d %d to L\n", i, j);
					}
		...
		/*for (int i = 0; i < numRows; i++)
		{
			for (int j = 0; j < numColumns; j++) printf("%c", input[i][j]);
			printf("\n");
		}*/

		for (int i = 0; i < numRows; i++) printf("%s\n", input[i]);
