...
int main(int argc, char* argv[])
{
	...
	while (change)
	{
		tempInput = (char**)malloc(sizeof(input));
		iterations++;
		printf("Iteration %d\n", iterations);
			...
			for (int j = 0; j < numColumns; j++)
			{
				...
				if (tempInput[i][j] == 'L')
				{
					for (int k = j - 1; k <= j + 1; k++)
					{
						...
						{
							if (k >= 0 && k < numColumns && l >= 0 && l < numRows && tempInput[k][l] == '#') foundOccupied = true;
						}
					}

					if (foundOccupied == false) input[i][j] = '#';
				}
				else if (tempInput[i][j] == '#')
				{
					...
					for (int k = j - 1; k <= j + 1; k++)
					{
						...
						{
							if (k == 0 && j == 0);
							else if (k >= 0 && k < numColumns && l >= 0 && l < numRows && tempInput[k][l] == '#') numOccupied++;
						}
					}

					if (numOccupied >= 4) input[i][j] = 'L';
